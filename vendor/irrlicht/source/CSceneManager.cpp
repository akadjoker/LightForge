...
namespace irr
{
	namespace scene
	{
			  ...
			  IRR_XML_FORMAT_SCENE(L"irr_scene"), IRR_XML_FORMAT_NODE(L"node"), IRR_XML_FORMAT_NODE_ATTR_TYPE(L"type")
		{
			...
			ShaderManager = new video::CShaderManager(Driver, FileSystem);
			...
			MaxDeltaTime = 1.0f / 15.0f;  // Máximo 15 FPS (evita saltos gigantes)
			MinDeltaTime = 1.0f / 300.0f; // Mínimo 300 FPS (evita divisão por zero)

			// História para smoothing
			HistorySize = 10;
			DeltaHistory.reallocate(HistorySize);
			for (u32 i = 0; i < HistorySize; ++i)
				DeltaHistory.push_back(TargetDeltaTime);
		...
		{
			os::Printer::log("Destroy SceneManager", ELL_INFORMATION);
			clearDeletionList();
															...
															bool makeActive)
		{
			...
			CCameraSceneNode *cam = new CCameraSceneNode(parent, this, id, position, lookat);
		...
		ISceneNode *CSceneManager::addCube(f32 size, ISceneNode *parent, s32 id, const core::vector3df &position, const core::vector3df &rotation, const core::vector3df &scale)
		{
		...
		ISceneNode *CSceneManager::addSphere(f32 radius, s32 polyCount, ISceneNode *parent, s32 id, const core::vector3df &position, const core::vector3df &rotation, const core::vector3df &scale)
		{
			if (!parent)
				parent = this;
		...
		void CSceneManager::update()
		{
			...
			f32 stableDeltaTime = calculateDeltaTime();


			OnAnimate(stableDeltaTime);
		...
		//	OnRegisterSceneNode();
			...
			// render default objects
			{
				...
				for (i = 0; i < SolidNodeList.size(); ++i)
				{
					// printf("Render Solid %d\n", SolidNodeList[i].Node->getID());
					SolidNodeList[i].Node->render();
				}

				// Parameters->setAttribute("drawn_solid", (s32) SolidNodeList.size());
				// printf("drawn_solid: %d\n", (s32) SolidNodeList.size());
				SolidNodeList.set_used(0);
			}
			...
			// render transparent objects.
			{
				...
				//	printf("drawn_transparent: %d\n", (s32) TransparentNodeList.size());
				TransparentNodeList.set_used(0);
			}
		...
		void CSceneManager::setDeltaLimits(f32 min, f32 max)
		{
			MinDeltaTime = min;
			MaxDeltaTime = max;
		}

		void CSceneManager::setHistorySize(u32 size)
		{
			HistorySize = size;
			DeltaHistory.clear();
			for (u32 i = 0; i < HistorySize; ++i)
		...
		f32 CSceneManager::getAverageDeltaTime() const
		{
			f32 sum = 0.0f;
			u32 count = HistoryFull ? HistorySize : CurrentHistoryIndex;

			for (u32 i = 0; i < count; ++i)
				sum += DeltaHistory[i];

			return count > 0 ? sum / count : TargetDeltaTime;
		}

		f32 CSceneManager::calculateDeltaTime()
		{

			u32 currentTime = os::Timer::getTime();

			// Calcular delta raw
			f32 rawDelta = (currentTime - LastTime) / 1000.0f; // ms para segundos
			LastTime = currentTime;

			// Aplicar limites de segurança
			rawDelta = core::clamp(rawDelta, MinDeltaTime, MaxDeltaTime);

			if (UseFixedTimeStep)
			{
				return getFixedDeltaTime(rawDelta);
			}
			else
			{
				return getSmoothedDeltaTime(rawDelta);
			}
		}

		f32 CSceneManager::getFixedDeltaTime(f32 rawDelta)
		{
			...
			return 0.0f; // Não atualizar este frame
		}

		f32 CSceneManager::getSmoothedDeltaTime(f32 rawDelta)
		{
			// Adicionar à história
			DeltaHistory[CurrentHistoryIndex] = rawDelta;
			CurrentHistoryIndex = (CurrentHistoryIndex + 1) % HistorySize;

			if (CurrentHistoryIndex == 0)
				HistoryFull = true;

			// Calcular média dos últimos N frames
			f32 sum = 0.0f;
			u32 count = HistoryFull ? HistorySize : CurrentHistoryIndex;
			...
			f32 smoothedDelta = sum / count;

			// Aplicar smoothing adicional se necessário
			const f32 smoothingFactor = 0.9f; // 0.0 = sem smooth, 1.0 = máximo smooth
			static f32 lastSmoothedDelta = smoothedDelta;

			smoothedDelta = (smoothedDelta * (1.0f - smoothingFactor)) +
							(lastSmoothedDelta * smoothingFactor);
