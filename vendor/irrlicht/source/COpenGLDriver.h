...
namespace irr
{
	...
	namespace video
	{
		...
		//! OpenGL implementation of vertex buffer (VBO)
		class COpenGLHardwareVertexBuffer : public video::CHardwareVertexBuffer
		{
		private:
			GLuint VBO;
		...
		public:
			COpenGLHardwareVertexBuffer(COpenGLDriver *driver,
										u32 vertexSize,
										u32 numVertices,
										video::E_USAGE usage);

			virtual ~COpenGLHardwareVertexBuffer();

			virtual u32 getId() const override;

			//! Override unlock to trigger GPU upload
		...
		private:
			GLenum getGLUsage(video::E_USAGE usage);
		};

		class COpenGLHardwareIndexBuffer : public video::CHardwareIndexBuffer
		{
		private:
			GLuint EBO;
			COpenGLDriver *Driver;
		...
		public:
			COpenGLHardwareIndexBuffer(COpenGLDriver *driver,
									   video::E_INDEX_TYPE type,
									   u32 numIndexes,
									   video::E_USAGE usage);

			virtual ~COpenGLHardwareIndexBuffer();
		...
		private:
			...
			{
				GLenum type;
				GLint size;		// nº componentes
				bool isInteger; // usa glVertexAttribIPointer
				bool isDouble;	// usa glVertexAttribLPointer
				GLboolean normalized;
			};
			
		// COpenGLVertexArray.h
		class COpenGLVertexArray : public IVertexArray
		{
		public:
			COpenGLVertexArray(COpenGLDriver *driver, IHardwareVertexBuffer *vb,  IHardwareIndexBuffer *ib);
			~COpenGLVertexArray() override;

			IVertexDescriptor *getVertexDescriptor() override { return Descriptor; }

			void bind() override;
			void unbind() override;

			bool isValid() const override { return VAO != 0; }
			void rebuild() override { update(); }
		...
		private:
			...
			GLTypeInfo toGLType(E_VERTEX_ATTRIBUTE_TYPE t, u32 elemCount) const;

		private:
			COpenGLDriver *Driver = nullptr;
			IVertexDescriptor *Descriptor = nullptr;
			IHardwareVertexBuffer* VB=nullptr; 
			IHardwareIndexBuffer * IB = nullptr;
			GLuint VAO = 0;
		};
...
//! Get Cg context
		...
		private:
			...
			IHardwareIndexBuffer *createIndexBuffer(video::E_INDEX_TYPE type, u32 numIndexes, video::E_USAGE usage) _IRR_OVERRIDE_;
			IHardwareVertexBuffer *createVertexBuffer(u32 vertexSize, u32 numVertices, video::E_USAGE usage) _IRR_OVERRIDE_;
