	...
	namespace core
	{
		...
		public:
			...
			*/
			...
			void fromEuler(const core::vector3df &axis) ;
		    void fromEulerZYX(const core::vector3df &euler);
		    void fromEulerXYZ(const core::vector3df &euler);
		...
		*/
		...
		// Converte ângulos de Euler (em graus) para quaternion
		// axis.X = pitch (rotação em X - olhar para cima/baixo)
		// axis.Y = yaw   (rotação em Y - olhar esquerda/direita)
		// axis.Z = roll  (rotação em Z - inclinar cabeça)
		inline void quaternion::fromEuler(const core::vector3df &euler)  
		{
			f64 pitch = euler.X * core::DEGTORAD64;
			f64 yaw = euler.Y * core::DEGTORAD64;
			f64 roll = euler.Z * core::DEGTORAD64;

			// Calcula senos e cosenos das metades dos ângulos
			f64 cy = cos(yaw * 0.5);
			f64 sy = sin(yaw * 0.5);
			...
			// Ordem de rotação: YXZ (Yaw -> Pitch -> Roll)
			// Esta é a ordem mais comum usada em games (tipo Unity)
			W = (f32)(cr * cp * cy + sr * sp * sy);
			X = (f32)(sr * cp * cy - cr * sp * sy);
			Y = (f32)(cr * sp * cy + sr * cp * sy);
			Z = (f32)(cr * cp * sy - sr * sp * cy);
		}
        

		// Ordem ZYX (Roll -> Yaw -> Pitch) - comum em aviões
		inline void quaternion::fromEulerZYX(const core::vector3df &euler)
		{
			f64 pitch = euler.X * core::DEGTORAD64;
			f64 yaw = euler.Y * core::DEGTORAD64;
			f64 roll = euler.Z * core::DEGTORAD64;
			...
			W = (f32)(cr * cp * cy + sr * sp * sy);
			X = (f32)(cr * sp * cy + sr * cp * sy);
			Y = (f32)(cr * cp * sy - sr * sp * cy);
			Z = (f32)(sr * cp * cy - cr * sp * sy);
		}

		// Ordem XYZ (Pitch -> Yaw -> Roll) - Irrlicht tradicional
		inline void quaternion::fromEulerXYZ(const core::vector3df &euler)
		{
			f64 pitch = euler.X * core::DEGTORAD64;
			f64 yaw = euler.Y * core::DEGTORAD64;
			f64 roll = euler.Z * core::DEGTORAD64;
